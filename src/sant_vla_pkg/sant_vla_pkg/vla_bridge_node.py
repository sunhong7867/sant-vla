...
def inference_request_kind(queued, threshold, now, last_preview, period, rate, latency):
    """Control refills have priority; extra display requests use spare time."""
    if queued < threshold:
        return "control"
    if (period > 0 and now-last_preview >= period
            and (queued-threshold)/rate > max(.3, 2*latency)):
        return "preview"
    return None
...
class VlaBridge(Node):
    ...
    def __init__(self):
        ...
        # Display-only fresh predictions. Zero preserves evaluation scheduling.
        self.display_preview_period = float(self.declare_parameter(
            "display_preview_period", 0.0).value)
        ...
        # Tier-scheduled curvature gain (classical speed scheduling, keyed on
        # the COMMANDED tier word because the model drives a different line
        # per sentence, not per physical speed — measured 2026-09-14: each
        # tier has its own optimum, slow/fast ≈0.85 vs normal 0.95). <=0
        # disables a tier override and curv_boost applies to every sentence.
        self.curv_boost_slow = float(
            self.declare_parameter("curv_boost_slow", -1.0).value)
        ...
        self.execution_contract = {
            ...
            "display_preview_period": self.display_preview_period,
        ...
        if self.goal_conditioning or self.display_preview_period > 0:
        ...
        self.preview_pub = self.create_publisher(String, "/vla/preview", 5)
        ...
        self._avoid_hold = False
        self.create_subscription(
            String, "/vla/avoidance_hold", self._avoid_hold_cb,
    ...
    def _avoid_hold_cb(self, msg):
        want = msg.data.strip().lower() in ("1", "true", "on", "hold")
        if want != self._avoid_hold:
            self._avoid_hold = want
            self.get_logger().info(f"avoidance hold -> {'ON' if want else 'off'}")
            if not want:
                self.queue.clear()
    ...
    def _instr_cb(self, msg):
        ...
        with self._task_lock:
            ...
            if changed:
                ...
                self.queue.clear()
                self.plan_pub.publish(String(data='{"actions": []}'))
                self.preview_pub.publish(String(data='{"actions": []}'))
        if changed:
            ...
            self.watchdog_hits = 0
            self._last_chunk_t = 0.0
            # The standstill channel restarts with the instruction, matching
            # the training distribution: to_lerobot counts rest seconds FROM
            # EPISODE START, so every training cruise begins at still=0 even
            # though the car was parked. Serving accumulated pre-instruction
            # idle time instead, and a spawn start with still≈cap lands in
            # the v9 GO-transition prior ("watched long enough — pull away
            # and change lanes"): the stochastic first-curve swerve measured
            # on 2026-09-14. A supervisor HOLD is not an instruction change,
            # so watch/red-light narration still sees the counter rise.
            self._still_s, self._still_t = 0.0, None
            low = text.lower()
            self._tier_boost = None
            if ("slow" in low or "gently" in low) and self.curv_boost_slow > 0:
                self._tier_boost = self.curv_boost_slow
    ...
    def _control(self):
        ...
        if self._hold or self._avoid_hold:
        ...
        if k_raw is not None:
            ...
            boost = self._tier_boost or self.curv_boost
            if (abs(boost - 1.0) > 1e-6
                ...
                gain = self.curv_gain_lo + (boost - self.curv_gain_lo) * t
    ...
    def _publish(self, v, w, override="none"):
        if self._floor > 0.0 and not (self._hold or self._avoid_hold) and override != "hold" \
                and v < self._floor:
    ...
    def _infer_loop(self):
        ...
        last_latency = .15
        while self._running:
            kind = inference_request_kind(
                len(self.queue), self.refill_at*self.chunk_len, time.monotonic(),
                last_preview, self.display_preview_period, self.rate_hz, last_latency)
            if kind is None:
                time.sleep(0.02)
                continue
            ...
            if kind == "preview" and seq == last_preview_seq:
                time.sleep(.02)
                continue
            origin = None
            if self._pose_stream and time.monotonic()-self._pose_stream.received_at < .3:
                origin = self._pose_stream.latest
            if self.goal_conditioning:
            ...
            req = msgpack.packb({"jpeg": jpeg, "state": state, "task": task,
                                 "seed": self.seed, "req": request_id if self.display_preview_period > 0 else self.n_infer,
            ...
            self.latencies.append(last_latency*1000.0)
            actions = rep.get("actions")
            if not actions:
                time.sleep(0.05)
                continue
            with self._task_lock:
                # A late reply to a previous instruction cannot refill the
                # control queue or resurrect its line after STOP/reset.
                if generation != self._task_generation or not self._running:
                    continue
                ...
                payload = json.dumps({"actions": actions, "origin": origin,
                    "yaw_to_heading": self.yaw_to_heading, "rate_hz": self.rate_hz,
                    "requested_at": t0, "t_mono": time.monotonic(), "task": task})
                if kind == "control":
                    self.queue.splice(actions)
                    self._last_chunk_t = time.monotonic()
                    self.plan_pub.publish(String(data=payload))
                    self.n_infer += 1
                if self.display_preview_period > 0:
                    self.preview_pub.publish(String(data=payload))
                    self.n_preview += 1
