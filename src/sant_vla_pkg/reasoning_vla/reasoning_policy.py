...
class ReasoningSmolVLAPolicy(SmolVLAPolicy):
    ...
    def _reasoning_ce(self, prefix_embs, prefix_pad, prefix_att,
        ...
        targets[~attn[:, 1:]] = -100
        if weights is None:
            # token-mean, byte-identical to the r1-r11 objective
            return F.cross_entropy(
                logits.reshape(-1, logits.shape[-1]), targets.reshape(-1),
                ignore_index=-100)
        # per-sample token-mean CE, then weight across the batch (r12
        # language-only boost: the sampler no longer over-represents rare
        # frames, this does — for the language head alone)
        tok = F.cross_entropy(
            logits.reshape(-1, logits.shape[-1]), targets.reshape(-1),
            ignore_index=-100, reduction="none").reshape(targets.shape)
        valid = (targets != -100).float()
        per_sample = (tok * valid).sum(1) / valid.sum(1).clamp(min=1)
    ...
    def forward(self, batch: dict, noise=None, time=None,
        ...
        action_weight = batch.get("action_weight")
        if action_weight is None:
            action_loss = losses.mean()
        else:
            # r13: obstacle-phase frames keep narrating (full CE weight)
            # but barely steer the expert — their swerve/stop trajectories
            # are what bent empty-track cruising inward (r11 0.88 m /
            # r12 0.61 m vs v3y 0.25 m median lane offset).
            per_sample = losses.mean(dim=(1, 2))
            action_weight = action_weight.to(per_sample.dtype)
            action_loss = ((per_sample * action_weight).sum()
                           / action_weight.sum().clamp(min=1e-8))

        # ----- reasoning pass -----
        ce = self._reasoning_ce(prefix_embs, prefix_pad, prefix_att, texts,
                                weights=batch.get("reasoning_weight"))
