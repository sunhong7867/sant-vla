...
#
#   bash tools/collect_v10.sh          # 본수집 36그룹(72ep) + heldout 6그룹
#   V10_GROUPS=6 V10_HELDOUT=0 bash tools/collect_v10.sh   # 스모크
WS=/home/sh/ROS2_project/sant-vla
OUT=$WS/eval_out/v10
DATA=$WS/src/sant_vla_pkg/data_v10
NGROUPS=${V10_GROUPS:-36}
NHELD=${V10_HELDOUT:-6}
# v9 교훈 재발(2026-09-14: 스모크가 1000그룹 계획으로 폭주) — 상한 가드.
[ "$NGROUPS" -le 60 ] 2>/dev/null || { echo "[v10] NGROUPS=$NGROUPS > 60 — 거부"; exit 2; }
[ "$NHELD" -le 20 ] 2>/dev/null || { echo "[v10] NHELD=$NHELD > 20 — 거부"; exit 2; }
SEED=${V10_SEED:-20260914}
mkdir -p "$OUT" "$DATA"
source /opt/ros/jazzy/setup.bash
source $WS/install/setup.bash

AVAIL=$(awk '/MemAvailable/ {print int($2/1048576)}' /proc/meminfo)
[ "$AVAIL" -ge 3 ] || { echo "[v10] 가용 RAM ${AVAIL}G < 3G — 앱을 닫고 재시도"; exit 4; }
...
[ "${N:-0}" -ge 1 ] || { echo "[v10] SIM_FAIL"; exit 3; }

echo "[v10] 오라클(lg=0.6)+레코더 기동..."
...
run_batch() {  # $1 groups  $2 prefix  $3 split  $4 pack_dir
  ...
  python3 $WS/src/sant_vla_pkg/scripts/collect_corpus.py \
    ...
    --cruise-lane both --split "$3" \
    --group-prefix "$2" --seed "$SEED" --out-dir "$DATA" \
    >> "$OUT/collect.log" 2>&1
  RC=$?
  echo "[v10] 배치 종료 rc=$RC"
  tail -6 "$OUT/collect.log"
  [ "$RC" -eq 0 ] || return "$RC"
  SESS=$(ls -dt "$DATA"/session_* 2>/dev/null | head -1)
  [ -n "$SESS" ] || { echo "[v10] 세션 없음"; return 5; }
  echo "[v10] finalize: $SESS -> $4"
  PACK_OUT=$4 bash $WS/src/sant_vla_pkg/scripts/finalize_corpus.sh "$SESS" \
    >> "$OUT/finalize.log" 2>&1 \
    && echo "[v10] finalize 완료 -> $4" \
    || { echo "[v10] finalize 실패 — $OUT/finalize.log"; return 6; }
}

run_batch "$NGROUPS" "v10_" train "$DATA/packed_v10" || exit $?
if [ "$NHELD" -gt 0 ]; then
