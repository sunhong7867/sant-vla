...
OUT=$WS/eval_out/lgpilot
DATA=$WS/src/sant_vla_pkg/data_lgpilot
NGROUPS=${LG_GROUPS:-6}
NHELD=0
LG=${LG:-0.4}
# v9 교훈 재발(2026-09-14: 스모크가 1000그룹 계획으로 폭주) — 상한 가드.
[ "$NGROUPS" -le 60 ] 2>/dev/null || { echo "[lgp] NGROUPS=$NGROUPS > 60 — 거부"; exit 2; }
[ "$NHELD" -le 20 ] 2>/dev/null || { echo "[lgp] NHELD=$NHELD > 20 — 거부"; exit 2; }
...
[ "$AVAIL" -ge 3 ] || { echo "[lgp] 가용 RAM ${AVAIL}G < 3G — 앱을 닫고 재시도"; exit 4; }
...
[ "${N:-0}" -ge 1 ] || { echo "[lgp] SIM_FAIL"; exit 3; }

echo "[lgp] 오라클(lg=0.6)+레코더 기동..."
setsid nohup ros2 run sant_vla_pkg route_oracle_node --ros-args \
  -p use_sim_time:=true -p lookahead_gain:=$LG -p controller:=${CTRL:-pure_pursuit} -p stanley_k:=${SK:-2.0} \
...
run_batch() {  # $1 groups  $2 prefix  $3 split  $4 pack_dir
  ...
  python3 $WS/src/sant_vla_pkg/scripts/collect_corpus.py \
    ...
    >> "$OUT/collect.log" 2>&1
  RC=$?
  echo "[lgp] 배치 종료 rc=$RC"
  tail -6 "$OUT/collect.log"
  [ "$RC" -eq 0 ] || return "$RC"
  SESS=$(ls -dt "$DATA"/session_* 2>/dev/null | head -1)
  [ -n "$SESS" ] || { echo "[lgp] 세션 없음"; return 5; }
  echo "[lgp] finalize: $SESS -> $4"
  PACK_OUT=$4 bash $WS/src/sant_vla_pkg/scripts/finalize_corpus.sh "$SESS" \
    >> "$OUT/finalize.log" 2>&1 \
    && echo "[lgp] finalize 완료 -> $4" \
    || { echo "[lgp] finalize 실패 — $OUT/finalize.log"; return 6; }
}

run_batch "$NGROUPS" "lg_" train "$DATA/packed_lg" || exit $?
if [ "$NHELD" -gt 0 ]; then
