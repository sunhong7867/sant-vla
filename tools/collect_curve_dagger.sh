...
#   V10_GROUPS=6 V10_NHELD=0 bash tools/collect_v10p.sh   # 스모크
WS=/home/sh/ROS2_project/sant-vla
OUT=$WS/eval_out/v10pd
DATA=$WS/src/sant_vla_pkg/data_v10pd
NGROUPS=24
NHELD=0
# v9 교훈 재발(2026-09-14: 스모크가 1000그룹 계획으로 폭주) — 상한 가드.
[ "$NGROUPS" -le 60 ] 2>/dev/null || { echo "[cd] NGROUPS=$NGROUPS > 60 — 거부"; exit 2; }
[ "$NHELD" -le 20 ] 2>/dev/null || { echo "[cd] NHELD=$NHELD > 20 — 거부"; exit 2; }
SEED=${V10_SEED:-20260915}
POSES=${DAGGER_POSES:-$WS/eval_out/rprobe/curve_dagger_starts.json}
mkdir -p "$OUT" "$DATA"
source /opt/ros/jazzy/setup.bash
source $WS/install/setup.bash

AVAIL=$(awk '/MemAvailable/ {print int($2/1048576)}' /proc/meminfo)
[ "$AVAIL" -ge 3 ] || { echo "[cd] 가용 RAM ${AVAIL}G < 3G — 앱을 닫고 재시도"; exit 4; }
...
[ "${N:-0}" -ge 1 ] || { echo "[cd] SIM_FAIL"; exit 3; }

echo "[cd] 오라클(lg=0.6)+레코더 기동..."
setsid nohup ros2 run sant_vla_pkg route_oracle_node --ros-args \
  -p use_sim_time:=true -p lookahead_gain:=0.6 \
...
run_batch() {  # $1 groups  $2 prefix  $3 split  $4 pack_dir
  echo "[cd] 수집 시작 ($(date +%H:%M)) — $1그룹 ($3)..."
  python3 $WS/src/sant_vla_pkg/scripts/collect_corpus.py \
    --driver oracle --groups 0 --speed-groups 0 --floor-groups 0 \
    --ring-groups 0 --obstacle-groups 0 --cruise-groups 0 \
    --start-poses-file "$POSES" --split "$3" \
    --group-prefix "$2" --seed "$SEED" --out-dir "$DATA" \
    >> "$OUT/collect.log" 2>&1
  RC=$?
  echo "[cd] 배치 종료 rc=$RC"
  ...
  [ -n "$SESS" ] || { echo "[cd] 세션 없음"; return 5; }
  echo "[cd] finalize: $SESS -> $4"
  PACK_OUT=$4 bash $WS/src/sant_vla_pkg/scripts/finalize_corpus.sh "$SESS" \
    >> "$OUT/finalize.log" 2>&1 \
    && echo "[cd] finalize 완료 -> $4" \
    || { echo "[cd] finalize 실패 — $OUT/finalize.log"; return 6; }
}

run_batch "$NGROUPS" "cd_" train "$DATA/packed_v10pd" || exit $?
if [ "$NHELD" -gt 0 ]; then
