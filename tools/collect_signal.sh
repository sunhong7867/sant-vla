...
OUT=$WS/eval_out/signal
DATA=$WS/src/sant_vla_pkg/data_signal
NGROUPS=${SIG_GROUPS:-3}
NHELD=0
LG=${LG:-0.4}
# v9 교훈 재발(2026-09-14: 스모크가 1000그룹 계획으로 폭주) — 상한 가드.
[ "$NGROUPS" -le 130 ] 2>/dev/null || { echo "[sig] NGROUPS=$NGROUPS > 60 — 거부"; exit 2; }
[ "$NHELD" -le 20 ] 2>/dev/null || { echo "[sig] NHELD=$NHELD > 20 — 거부"; exit 2; }
...
[ "$AVAIL" -ge 3 ] || { echo "[sig] 가용 RAM ${AVAIL}G < 3G — 앱을 닫고 재시도"; exit 4; }
...
[ "${N:-0}" -ge 1 ] || { echo "[sig] SIM_FAIL"; exit 3; }

echo "[sig] 오라클(lg=0.6)+레코더 기동..."
...
run_batch() {  # $1 groups  $2 prefix  $3 split  $4 pack_dir
  echo "[sig] 수집 시작 ($(date +%H:%M)) — $1그룹 ($3)..."
  python3 $WS/src/sant_vla_pkg/scripts/collect_corpus.py \
    --driver oracle --groups 0 --speed-groups 0 --floor-groups 0 \
    --ring-groups 0 --obstacle-groups 0 --cruise-groups 0 --signal-groups "$1" \
    --split "$3" \
    --group-prefix "$2" --seed "$SEED" --out-dir "$DATA" --lat-jitter 0.0 --yaw-jitter 0 \
    >> "$OUT/collect.log" 2>&1
  RC=$?
  echo "[sig] 배치 종료 rc=$RC"
  tail -6 "$OUT/collect.log"
  [ "$RC" -eq 0 ] || return "$RC"
  SESS=$(ls -dt "$DATA"/session_* 2>/dev/null | head -1)
  [ -n "$SESS" ] || { echo "[sig] 세션 없음"; return 5; }
  echo "[sig] resample+package (bypass counterfactual pair-check): $SESS -> $4"
  # The signal axis pairs red-stop vs green-pass, which differ on OUTCOME as
  # well as light colour, so the CMI pair-verify rejects them. Training does
  # not need pairs — resample and package the episodes directly.
  python3 $WS/src/sant_vla_pkg/scripts/resample_episodes.py --session "$SESS" \
    --yaw-offset-deg -90 --pose-source tf >> "$OUT/finalize.log" 2>&1
  rm -rf "$4"
  python3 $WS/src/sant_vla_pkg/scripts/package_corpus.py "$4" "$SESS" \
    >> "$OUT/finalize.log" 2>&1 \
    && echo "[sig] package 완료 -> $4" \
    || { echo "[sig] package 실패 — $OUT/finalize.log"; return 6; }
}

run_batch "$NGROUPS" "sig_" train "$DATA/packed_signal" || exit $?
if [ "$NHELD" -gt 0 ]; then
