...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'output/blender/parking_v2';FRAMES=OUT/'frames';FRAMES.mkdir(exist_ok=True)
...
for i,r in enumerate(p['frames']):
 if not r['hold'] and i%2:continue
 pose=tuple(r['xy'])+(r['yaw'],)
...
s.frame_set(1);s['Parking animation']='Saved IN pose -> perpendicular bay -> 5.5 s stationary -> parallel bay -> 5.5 s stationary -> OUT. Steering-style path, illustrative animation.'
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'parking_animated.blend'))
# Reuse only identical poses from the verified first video, with the same camera.
import shutil
old=ROOT/'output/blender/parking'
oldmeta=json.loads((old/'metadata.json').read_text())
def posekey(r):return tuple(round(v,6) for v in r['xy'])+(round(r['yaw']%(2*math.pi),6),)
reuse={posekey(r):r['render'] for r in oldmeta['frames']}
for r in selected:
 src=old/'frames'/f"parking_{reuse.get(posekey(r),-1):04d}.png"
 dest=FRAMES/f"parking_{r['render']:04d}.png"
