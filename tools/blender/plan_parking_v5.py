...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'output/blender/parking_v5'
...
pstart=None
for rp in np.arange(4.5,3.79,-.1):
...
for back in np.arange(0,1.51,.05):
 ...
 for rx in np.arange(4.5,3.79,-.1):
...
def move(d,k,label,speed=1.5):
 global q,lastk
 if abs(k-lastk)>1e-8:pause(1.5,'정지 · 부드러운 앞바퀴 조향',False,k)
...
move(17.9385-REAR-q[1],0,'OUT 직진 통과 · 미션 완료',2)
records=[];distance=0
for seg in segments:
