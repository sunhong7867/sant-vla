...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'output/blender/parking'
...
a=np.array([start[0],-8]);b=np.array([-3.,9.4]);end=np.array([-1.9174,17.9385])
segments=[]
...
add(p,8,'오른쪽 수직주차 · 진입');hold(5.5,'수직주차 · 정지 유지',0)
add(p[::-1],8,'수직주차 완료 · 후진 출차',True,1)
hold(.6,'전진 전환',1)
add([a,a+[0,6],b-[0,6],b],9,'왼쪽 평행주차 · 접근',False,1)
hold(.6,'후진 전환',1)
