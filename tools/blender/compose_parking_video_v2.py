...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'output/blender/parking_v2';meta=json.loads((OUT/'metadata.json').read_text());BLUE=COLORS[0];GREEN=(71,229,132)
cache={}
def frame(i,intro=False,outro=False):
 ...
 d.rounded_rectangle((1110,18,1380,62),radius=12,fill=CARD);txt(d,(1127,29),'정차 실시간' if r['hold'] else '주행 2배속 · 연출',21)
 ...
 if r['hold']:
  ...
  txt(d,(1445,891),'연속 주행 · 순서대로 수행',23,MUTED);txt(d,(1445,934),'수직주차 후 평행주차',25)
 ...
 d.rectangle((0,1030,1920,1080),fill=BG);txt(d,(30,1046),'규정 ver3.1  §4.8  |  Blender 경로 애니메이션 · 실제 제어 기록이 아닌 설명용 예시',20,MUTED)
 return im
if __name__=='__main__':
 ...
 if '--preview' in sys.argv:
  ...
  proc=subprocess.Popen(['ffmpeg','-y','-loglevel','error','-f','rawvideo','-vcodec','rawvideo','-pix_fmt','rgb24','-s','1920x1080','-r','24','-i','-','-an','-c:v','libx264','-preset','fast','-crf','19','-threads','4','-pix_fmt','yuv420p','-movflags','+faststart',str(OUT/'parking_driving.mp4')],stdin=subprocess.PIPE)
  intro=frame(0,intro=True).tobytes()
  for _ in range(48):proc.stdin.write(intro)
  ...
  print('VIDEO_COMPLETE',(len(meta['frames'])+120)/24)
